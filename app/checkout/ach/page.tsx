...
function AchPaymentContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const orderId = searchParams.get("order_id");
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [bankDetails, setBankDetails] = useState<AchBankDetails | null>(null);
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-4xl">
      <h1 className="font-editorial text-4xl mb-8">ACH or Wire Transfer</h1>
          ...
          {orderInfo && usdAmount !== null && (
            <div className="p-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-300 rounded-lg">
              ...
              <p className="text-4xl font-bold text-blue-900">
                ${usdAmount.toFixed(2)} <span className="text-2xl">USD</span>
              </p>
              {exchangeRate && (
